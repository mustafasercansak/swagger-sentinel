...
export function activate(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		...
		vscode.commands.registerCommand("swagger-sentinel.generateTests", async () => {
			const editor = vscode.window.activeTextEditor;
			if (!editor) {
				vscode.window.showWarningMessage("Swagger Sentinel: No active file.");
				return;
		...
		vscode.workspace.onDidSaveTextDocument(async (doc) => {
			...
			if (result) statusBarManager.update(result.errorCount, result.warningCount);
		}),
		...
		vscode.window.onDidChangeActiveTextEditor((editor) => {
			if (!editor) { statusBarManager.clear(); return; }
			if (!diagnosticsProvider.isSpecFile(editor.document)) { statusBarManager.hide(); return; }
...
async function generateTests(doc: vscode.TextDocument): Promise<void> {
	...
	await vscode.window.withProgress(
		{ location: vscode.ProgressLocation.Notification, title: "Swagger Sentinel: Generating tests…" },
		async () => {
			try {
				...
				const rootPath = workspaceFolder?.uri.fsPath ?? path.dirname(doc.uri.fsPath);
				...
				vscode.window.showInformationMessage(`Swagger Sentinel: ${files.length} test file(s) generated in ${outputDir}`);
			} catch (err) {
