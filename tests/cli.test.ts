...
describe("CLI integration", () => {
	...
	beforeEach(() => {
		...
		fs.writeFileSync(INVALID_SPEC_PATH, `
openapi: 3.0.0
info:
  title: Invalid Spec
  version: ""
...
paths:
  /new-path:
	...
	it("should validate a valid spec", async () => {
		...
		expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("checks passed"));
	});

	it("should validate in JSON format", async () => {
		await run(["node", "cli.js", "validate", "tests/sample-spec.yaml", "--format", "json"]);
	...
	it("should output GitHub annotations", async () => {
		...
		await run(["node", "cli.js", "validate", INVALID_SPEC_PATH, "--github-annotations"]);
		expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("::error"));
		delete process.env.GITHUB_ACTIONS;
	});

	it("should save summary to file", async () => {
		const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((path, data, options) => {
			// Avoid actually writing the summary during test if we can
			if (typeof path === "string" && path.includes("summary.md")) return undefined;
			return undefined;
		});
		await run(["node", "cli.js", "validate", "tests/sample-spec.yaml", "--summary", "summary.md"]);
	...
	it("should generate tests for a spec", async () => {
		...
		await run(["node", "cli.js", "generate", "tests/sample-spec.yaml", "-o", "out"]);
		expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("Generated"));
	});

	it("should run diff command with no changes", async () => {
		await run(["node", "cli.js", "diff", "tests/sample-spec.yaml", "tests/sample-spec.yaml"]);
	...
	it("should run diff command and detect changes", async () => {
		await run(["node", "cli.js", "diff", "tests/sample-spec.yaml", V2_SPEC_PATH]);
	...
	it("should run syntax command", async () => {
		...
		expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("Valid OpenAPI"));
	});
	...
	it("should run list rules command with category filter", async () => {
		...
		expect(logSpy).not.toHaveBeenCalledWith(expect.stringContaining("Security"));
	});
