...
describe('validatePaths', () => {
  ...
  it('P26 passes for normal path params', () => {
    const s = spec({ '/users/{userId}': { get: { parameters: [{ name: 'userId', in: 'path', required: true, schema: { type: 'string' } }], responses: {} } } });
    expect(check(validatePaths(s), 'P26')?.passed).toBe(true);
  });

  it('P26 fails for "token" path param', () => {
    const s = spec({ '/users/{token}': { get: { parameters: [{ name: 'token', in: 'path', required: true, schema: { type: 'string' } }], responses: {} } } });
