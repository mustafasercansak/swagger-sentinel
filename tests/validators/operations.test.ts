...
describe('validateOperations', () => {
  ...
  it('O42 fails when GET has requestBody', () => {
    const s = spec({ '/items': { get: { requestBody: { content: { 'application/json': {} } }, responses: { '200': { description: 'ok' } } } } });
  ...
  it('O43 passes when 429 has Retry-After', () => {
    const s = spec({ '/items': { get: { responses: { '429': { description: 'too many', headers: { 'Retry-After': { schema: { type: 'integer' } } } } } } } });
    expect(check(validateOperations(s), 'O43')?.passed).toBe(true);
  });

  it('O43 fails when 429 missing rate-limit headers', () => {
    const s = spec({ '/items': { get: { responses: { '429': { description: 'too many' } } } } });
  ...
  it('O44 passes when 202 has Location', () => {
    const s = spec({ '/jobs': { post: { responses: { '202': { description: 'queued', headers: { 'Location': { schema: { type: 'string' } } } } } } } });
    expect(check(validateOperations(s), 'O44')?.passed).toBe(true);
  });

  it('O44 fails when 202 missing Location/Link', () => {
    const s = spec({ '/jobs': { post: { responses: { '202': { description: 'queued' } } } } });
