...
describe('validateSecurity', () => {
  ...
  it('SEC101 passes when header name is normal', () => {
    const s = spec({ components: { securitySchemes: { apiKey: { type: 'apiKey', in: 'header', name: 'Authorization' } } } });
    expect(check(validateSecurity(s as any), 'SEC101')?.passed).toBe(true);
  });

  it('SEC101 fails when header name starts with X-', () => {
    const s = spec({ components: { securitySchemes: { apiKey: { type: 'apiKey', in: 'header', name: 'X-API-TOKEN' } } } });
        ...
        '/index.html': {
          get: {
            responses: {
              '200': {
                content: { 'text/html': {} },
                headers: { 'Content-Security-Policy': { schema: { type: 'string' } } },
              },
            },
          },
        },
