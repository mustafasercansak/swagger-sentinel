...
export function validatePaths(spec: OpenAPISpec): ValidationResult[] {
	...
	results.push({
		...
		details:
			nonKebab.length > 0 ? `Non-kebab paths: ${nonKebab.join(", ")}` : null,
	});
	...
	results.push({
		...
		details:
			trailingSlash.length > 0 ? `Found: ${trailingSlash.join(", ")}` : null,
	});
	...
	results.push({
		...
		details:
			singularSuspects.length > 0
				? `Possibly singular: ${[...new Set(singularSuspects)].join(", ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			deepPaths.length > 0 ? `Deep paths: ${deepPaths.join(", ")}` : null,
	});

	// P19: Versioned paths
	const hasVersionedPaths = paths.some((p) => /\/v\d+\//.test(p));
	const hasServerVersioning = (spec.servers || []).some((s) =>
		/\/v\d+/.test(s.url || ""),
	);
	results.push({
		id: "P19",
	...
	const extPaths = paths.filter((p) =>
		/\.\w{2,4}$/.test(p.replace(/\{[^}]+\}/g, "")),
	);
	results.push({
		id: "P20",
		category: "Paths",
	...
	results.push({
		...
		details:
			uniquePrefixes.length > 3
				? `Found ${uniquePrefixes.length} different prefixes`
				: null,
	});
	...
	results.push({
		...
		details:
			emptySegments.length > 0 ? `Found: ${emptySegments.join(", ")}` : null,
	});

	// P23: Path parameters are documented
	const undocumentedParams: string[] = [];
	for (const op of ops) {
		const pathParams = (op.path.match(/\{(\w+)\}/g) || []).map((p: string) =>
			p.replace(/[{}]/g, ""),
		);
	...
	results.push({
		...
		details:
			undocumentedParams.length > 0
				? `Missing: ${undocumentedParams.slice(0, 3).join("; ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			verbsInPaths.length > 0
				? `Verb in path: ${verbsInPaths.slice(0, 3).join(", ")}`
				: null,
	});
	...
	results.push({
		...
		message:
			"Path parameter names use consistent casing (not mixed camelCase and snake_case)",
		details:
			hasCamel && hasSnake
				? `Mixed casing detected in: ${[...new Set(allParamNames)].join(", ")}`
				: null,
	});
	...
	results.push({
		...
		message:
			"Path parameters should not contain sensitive keywords (key, token, secret, auth, password)",
		details:
			sensitiveParams.length > 0
				? `Sensitive params: ${sensitiveParams.slice(0, 3).join("; ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			dotSegments.length > 0
				? `Found trailing dots in: ${dotSegments.join(", ")}`
				: null,
	});
