...
const FORMAT_HINTS = [
	...
	{
		fragments: ["date_of_birth", "dob", "birth_date", "birthdate"],
		format: "date",
	},
	{
		fragments: ["created_at", "updated_at", "deleted_at", "timestamp"],
		format: "date-time",
	},
...
export function validateRequests(spec: OpenAPISpec): ValidationResult[] {
	...
	for (const op of ops) {
		...
		for (const param of params) {
			...
			if (schema.enum) {
				const hasUpper = schema.enum.some(
					(e) =>
						typeof e === "string" &&
						e === e.toUpperCase() &&
						e !== e.toLowerCase(),
				);
				const hasLower = schema.enum.some(
					(e) =>
						typeof e === "string" &&
						e === e.toLowerCase() &&
						e !== e.toUpperCase(),
				);
				const hasMixed = schema.enum.some(
					(e) =>
						typeof e === "string" &&
						e !== e.toUpperCase() &&
						e !== e.toLowerCase(),
				...
				if (schema.properties) {
					for (const [propName, propSchema] of Object.entries(
						...
						if (
							...
							stringParamsNoMax.push(
								`${op.method} ${op.path} body.${propName}`,
							);
						}
	...
	results.push({
		...
		details:
			stringParamsNoMax.length > 0
				? `Missing maxLength: ${stringParamsNoMax.slice(0, 3).join("; ")}${stringParamsNoMax.length > 3 ? ` (+${stringParamsNoMax.length - 3} more)` : ""}`
				: null,
	});
	...
	results.push({
		...
		details:
			numParamsNoRange.length > 0
				? `Missing range: ${numParamsNoRange.slice(0, 3).join("; ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			arrayParamsNoMax.length > 0
				? `Missing maxItems: ${arrayParamsNoMax.slice(0, 3).join("; ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			enumInconsistencies.length > 0
				? `Inconsistent: ${enumInconsistencies.slice(0, 2).join("; ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			missingRequired.length > 0
				? `No required: ${missingRequired.join(", ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			noContentType.length > 0
				? `Missing content type: ${noContentType.join(", ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			paramNoDesc.length > 0
				? `Missing: ${paramNoDesc.slice(0, 3).join("; ")}${paramNoDesc.length > 3 ? ` (+${paramNoDesc.length - 3} more)` : ""}`
				: null,
	});
	...
	function checkFormatHints(
		propName: string,
		propSchema: OpenAPISchema,
		loc: string,
	) {
		if (propSchema.type !== "string" || propSchema.format || propSchema.enum)
			return;
	...
	results.push({
		...
		details:
			formatMismatches.length > 0
				? `Missing format: ${formatMismatches.slice(0, 3).join("; ")}${formatMismatches.length > 3 ? ` (+${formatMismatches.length - 3} more)` : ""}`
				: null,
	});
			...
			const hasBinary =
				...
				(schema.properties &&
					Object.values(schema.properties).some(
						(p) => p.format === "binary" || p.format === "byte",
					));
			if (hasBinary) {
	...
	results.push({
		...
		details:
			binaryNotMultipart.length > 0
				? `Binary outside multipart: ${binaryNotMultipart.join(", ")}`
				: null,
	});
	...
	for (const op of ops) {
		...
		for (const param of params) {
			...
			if (
				(name.includes("id") || name.includes("uuid")) &&
				param.schema &&
				!param.schema.format &&
	...
	results.push({
		...
		details:
			idMissingFormat.length > 0
				? `Missing format/pattern: ${idMissingFormat.slice(0, 3).join("; ")}`
				: null,
	});

	// R60: Large objects should suggest maxProperties
	const largeObjectsNoMax: string[] = [];
	function checkLargeObject(schema: OpenAPISchema, loc: string) {
		if (
			schema.type === "object" &&
			schema.properties &&
			Object.keys(schema.properties).length > 10 &&
	...
	results.push({
		...
		details:
			largeObjectsNoMax.length > 0
				? `No maxProperties: ${largeObjectsNoMax.slice(0, 3).join(", ")}`
				: null,
	});
	...
	function checkSensitiveExamples(schema: OpenAPISchema, loc: string) {
	...
	results.push({
		...
		message:
			"Sensitive fields (password, token, etc.) should not include example values",
		details:
			sensitiveWithExample.length > 0
				? `Found examples in: ${sensitiveWithExample.slice(0, 3).join(", ")}`
				: null,
	});
