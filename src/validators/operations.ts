...
export function validateOperations(spec: OpenAPISpec): ValidationResult[] {
	...
	results.push({
		...
		details:
			dupes.length > 0 ? `Duplicates: ${[...new Set(dupes)].join(", ")}` : null,
	});
	...
	results.push({
		...
		details:
			postWith200.length > 0
				? `Found 200 on: ${postWith200.map((o) => o.path).join(", ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			deleteNon204.length > 0
				? `Non-204: ${deleteNon204.map((o) => o.path).join(", ")}`
				: null,
	});

	// O34: All operations have at least one tag
	const untagged = ops.filter(
		(o) => !o.operation.tags || o.operation.tags.length === 0,
	);
	results.push({
		id: "O34",
		category: "Operations",
	...
	results.push({
		...
		details:
			listOpsWithoutPagination.length > 0
				? `No pagination: ${listOpsWithoutPagination.join(", ")}`
				: null,
	});

	// O36: All operations have summary or description
	const undescribed = ops.filter(
		(o) => !o.operation.summary && !o.operation.description,
	);
	results.push({
		id: "O36",
		category: "Operations",
	...
	const putPatch = ops.filter(
		(o) => o.method === "PUT" || o.method === "PATCH",
	);
	...
	results.push({
		...
		details:
			postWithQuery.length > 0
				? `Mixed: ${postWithQuery.map((o) => o.path).join(", ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			getPathsWithoutHead.length > 0
				? `Missing HEAD: ${getPathsWithoutHead.slice(0, 3).join(", ")}${getPathsWithoutHead.length > 3 ? ` (+${getPathsWithoutHead.length - 3} more)` : ""}`
				: null,
	});
	...
	results.push({
		...
		details:
			patchWrongType.length > 0
				? `Unexpected content type: ${patchWrongType.map((o) => o.path).join(", ")}`
				: null,
	});
	...
	results.push({
		...
		details:
			verbInId.length > 0
				? `Redundant prefix: ${verbInId.slice(0, 3).join("; ")}`
				: null,
	});

	// O42: GET operations should not have a requestBody
	const getWithBody = ops.filter(
		(o) => o.method === "GET" && o.operation.requestBody,
	);
	results.push({
		id: "O42",
		category: "Operations",
		...
		details:
			getWithBody.length > 0
				? `Found body on: ${getWithBody.map((o) => o.path).join(", ")}`
				: null,
	});
	...
	results.push({
		...
		message:
			"429 Too Many Requests responses include rate-limit or retry headers",
		details:
			rateLimitMissingHeaders.length > 0
				? `Missing headers: ${rateLimitMissingHeaders.join(", ")}`
				: null,
	});
	...
	results.push({
		...
		message:
			"202 Accepted responses include a Location or Link header for status polling",
		details:
			acceptedMissingHeaders.length > 0
				? `Missing headers: ${acceptedMissingHeaders.join(", ")}`
				: null,
	});
