...
function validateStructure(spec) {
  ...
  results.push({
    id: 'S01', category: 'Structure', severity: 'error',
    passed: !!info.contact,
  ...
  results.push({
    id: 'S02', category: 'Structure', severity: 'error',
    passed: !!info.version && /^\d+\.\d+\.\d+/.test(info.version),
    message: 'API version follows semver format',
    details: info.version && !/^\d+\.\d+\.\d+/.test(info.version) ? `Found "${info.version}" — use semver (e.g., 1.2.0)` : null,
  });

  results.push({
    id: 'S03', category: 'Structure', severity: 'warning',
    passed: !!(spec.servers && spec.servers.length > 0),
    message: 'Servers array is defined',
    details: !(spec.servers && spec.servers.length > 0) ? 'Without servers, generated SDKs have no base URL' : null,
  });

  results.push({
    id: 'S04', category: 'Structure', severity: 'error',
  ...
  results.push({
    id: 'S05', category: 'Structure', severity: 'error',
    passed: version.startsWith('3.0') || version.startsWith('3.1'),
    message: `OpenAPI version is 3.0.x or 3.1.x (found: ${version})`,
  });

  results.push({
    id: 'S06', category: 'Structure', severity: 'warning',
    passed: !!info.description,
    message: 'Info block has a description',
  });

  results.push({
    id: 'S07', category: 'Structure', severity: 'suggestion',
    passed: !!info.license,
    message: 'License field is specified',
    details: !info.license ? 'Important for public APIs — consumers need legal clarity' : null,
  });

  results.push({
    id: 'S08', category: 'Structure', severity: 'warning',
  ...
  results.push({
    id: 'S09', category: 'Structure', severity: 'suggestion',
