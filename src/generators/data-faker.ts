...
export function getFakerCall(propName: string, schema: any): string {
  ...
  // 2. Semantic mapping by property name
  ...
  if (name.includes('zipcode') || name.includes('postalcode')) return 'faker.location.zipCode()';
  ...
  if (name.includes('title') || name.includes('subject')) return 'faker.lorem.sentence()';
  ...
  switch (schema.type) {
    ...
    case 'array':
      const itemsCall = getFakerCall(propName + 'Item', schema.items || { type: 'string' });
